...
func TestJWTer_GetJWT(t *testing.T) {
	...
	want, err := jwt.NewBuilder().
		...
		Issuer(`github.com/mongolmongol2022/go_todo_app_HandsOnTraining`).
	...
	req := httptest.NewRequest(
		http.MethodGet,
		`https://github.com/mongolmongol2022`,
		nil,
	)
	req.Header.Set(`Authorization`, fmt.Sprintf(`Bearer %s`, signed))
...
func TestJWTer_GetJWT_NG(t *testing.T) {
	...
	for n, tt := range tests {
		...
		t.Run(n, func(t *testing.T) {
			...
			req := httptest.NewRequest(
				http.MethodGet,
				`https://github.com/mongolmognol2022`,
				nil,
			)
			req.Header.Set(`Authorization`, fmt.Sprintf(`Bearer %s`, signed))
			got, err := sut.GetToken(ctx, req)
